...
The `/menu` page's category navigation bar would visibly jump/shake during fast or momentum scrolling on iPhone/iOS Safari only — not on Android or desktop. This was fixed in two rounds:

**Round 1 — compositor promotion (kept, but not sufficient on its own)**

- `public/css/menu.css`: `.mnu-nav` (then still `position: sticky`) got `transform: translateZ(0)`, `will-change: transform`, and `backface-visibility: hidden` to force it onto its own GPU compositor layer.
- `components/MenuPage.tsx`: the nav auto-centering effect (the one that keeps the active category pill scrolled into view) now defers its `scrollLeft` read and `scrollTo` write into a `requestAnimationFrame` instead of running them synchronously inside the scroll-driven `IntersectionObserver` callback that tracks the active category — that synchronous read/write was forcing a layout in the middle of iOS's own scroll handling.

**Round 2 — replaced native `position: sticky` with JS-driven `position: fixed`**

Round 1 reduced but didn't eliminate the jitter on a real device. The actual root cause is a long-standing WebKit bug: `position: sticky` elements that paint their own background/border/box-shadow lag behind fast/momentum scrolling on iOS and visibly snap into place once the browser catches up — no CSS-only compositor hint fully fixes it. The site's own header sidesteps this because it's plain `position: fixed`, driven by JS, and doesn't show the bug. So the category bar is now pinned the same way:

- `public/css/menu.css`: `.mnu-nav` is `position: relative` by default (normal flow, same visual appearance as before). A new `.mnu-nav.is-pinned` rule switches it to `position: fixed; top: 96px/88px/78px` (same offsets as the old sticky breakpoints) only once JS adds that class. A new `.mnu-nav-spacer` element reserves the bar's layout space so nothing above or below it shifts when it leaves normal flow.
- `components/MenuPage.tsx`: a `ResizeObserver` effect keeps the spacer's height in sync with the nav bar's real rendered size. A passive, `requestAnimationFrame`-throttled scroll/resize listener measures the site's fixed header (`header.nav`) and toggles `.is-pinned` once the bar's spacer scrolls up to the header's edge — mirroring the exact "measure the real header, toggle a class" approach `Header.tsx` already uses for its own compact state.
- Both rounds are inert on Android and desktop — same visual position and trigger point, only the underlying positioning mechanism changed.
